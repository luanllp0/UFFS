...
meus_tokens, minhas_gramaticas = ler_arquivo_entrada('entradaLFA.txt')
...
# A seção abaixo implementa o algoritmo de reconhecimento léxico.
# Ela lê o arquivo de entrada, percorre cada cadeia, verifica se ela é aceita pelo AFD,
# grava a FITA e registra a Tabela de Símbolos com a linha, o identificador e o rótulo.

# A FITA guarda a sequência de identificadores reconhecidos e o rótulo da cadeia correspondente.
fita = list()

# A Tabela de Símbolos guarda cada token reconhecido com linha, identificador e rótulo.
tabela_simbolos = list()

# O conjunto de separadores define quando uma cadeia termina.
# Quando o programa encontra qualquer um desses caracteres, ele finaliza a cadeia atual.
separadores = {" ", "\t", "\n", ";", ",", "(", ")", "{", "}", ":", "=", "+", "-", "*", "/", "<", ">", "!"}


class Simbolo:
    # A classe Simbolo representa uma entrada da Tabela de Símbolos.
    def __init__(self, linha, identificador, rotulo):
        # A linha indica em qual linha do arquivo a cadeia foi encontrada.
        self.linha = linha
        # O identificador representa o estado final reconhecido pelo AFD.
        self.identificador = identificador
        # O rótulo guarda a cadeia original reconhecida, como "ei", "ou" ou "x".
        self.rotulo = rotulo


def salvar_fita_em_arquivo(nome_arquivo, fita):
    # Esta função salva a FITA em um arquivo de texto.
    # O objetivo é deixar a sequência de reconhecimentos em um arquivo separado.
    with open(nome_arquivo, 'w', encoding='utf-8') as arquivo:
        # Percorre cada token já armazenado na fita.
        for identificador, rotulo in fita:
            # Escreve uma linha no formato: identificador | rótulo.
            arquivo.write(f"{identificador} | {rotulo}\n")


def salvar_tabela_simbolos_em_arquivo(nome_arquivo, tabela_simbolos):
    # Esta função salva a Tabela de Símbolos em um arquivo de texto.
    # Cada linha do arquivo representa uma entrada da tabela.
    with open(nome_arquivo, 'w', encoding='utf-8') as arquivo:
        # Percorre cada entrada da tabela de símbolos.
        for simbolo in tabela_simbolos:
            # Escreve no formato: linha=... | identificador=... | rotulo=...
            arquivo.write(f"linha={simbolo.linha} | identificador={simbolo.identificador} | rotulo={simbolo.rotulo}\n")


def registrar_token(cadeia, identificador, linha):
    # Esta função registra uma cadeia reconhecida na FITA e na Tabela de Símbolos.
    # Ela evita registrar cadeias vazias.
    if not cadeia:
        return

    # Se o identificador atual não for um estado final do AFD, a cadeia deve ser tratada como erro.
    if identificador not in meu_afd_completo.estados_finais:
        identificador = 'Erro'

    # Adiciona o token reconhecido na FITA no formato (identificador, rótulo).
    fita.append((identificador, cadeia))
    # Adiciona o token reconhecido na Tabela de Símbolos com linha, identificador e rótulo.
    tabela_simbolos.append(Simbolo(linha=linha, identificador=identificador, rotulo=cadeia))


def reconhecer_arquivo(nome_arquivo):
    # Esta função percorre o arquivo de entrada e aplica o algoritmo de reconhecimento léxico.
    # Ela usa o AFD completo para decidir se cada cadeia é aceita ou rejeitada.

    # O estado atual começa no estado inicial do automato.
    identificador_corrente = 'S'

    # A variável linha_atual guarda em qual linha do arquivo o programa está lendo.
    linha_atual = 1

    # A variável cadeia_atual guarda a cadeia que está sendo montada no momento.
    cadeia_atual = ''

    # Abre o arquivo de entrada em modo de leitura.
    with open(nome_arquivo, 'r', encoding='utf-8') as arquivo:
        # Lê todas as linhas do arquivo para processá-las uma por uma.
        linhas = arquivo.readlines()

        # Percorre cada linha do arquivo.
        for linha in linhas:
            # Percorre cada símbolo da linha.
            for simbolo in linha:
                # Se o símbolo lido for separador, então a cadeia atual terminou.
                if simbolo in separadores:
                    # Verifica se houve alguma cadeia acumulada antes do separador.
                    if cadeia_atual:
                        # Registra a cadeia reconhecida e reinicia a leitura da próxima cadeia.
                        registrar_token(cadeia_atual, identificador_corrente, linha_atual)
                        cadeia_atual = ''
                        identificador_corrente = 'S'
                else:
                    # Se o símbolo não é separador, ele faz parte da cadeia atual.
                    # Então o programa consulta o AFD para saber o próximo estado.
                    if identificador_corrente in meu_afd_completo.transicoes and simbolo in meu_afd_completo.transicoes[identificador_corrente]:
                        # O estado corrente avança para o próximo estado indicado pela transição.
                        identificador_corrente = next(iter(meu_afd_completo.transicoes[identificador_corrente][simbolo]))
                    else:
                        # Se não existir transição para esse símbolo, o automato vai para o estado de erro.
                        identificador_corrente = 'Erro'
                    # Acrescenta o símbolo à cadeia em construção.
                    cadeia_atual += simbolo

            # Ao final de cada linha do arquivo, avança para a próxima linha do código-fonte.
            linha_atual += 1

        # Depois de percorrer todas as linhas, ainda pode existir uma cadeia sem separador no final.
        if cadeia_atual:
            # Registra a última cadeia que ficou pendente no final do arquivo.
            registrar_token(cadeia_atual, identificador_corrente, linha_atual - 1)

    # Salva a FITA em um arquivo chamado fita.txt.
    salvar_fita_em_arquivo("fita.txt", fita)
    # Salva a Tabela de Símbolos em um arquivo chamado tabela_simbolos.txt.
    salvar_tabela_simbolos_em_arquivo("tabela_simbolos.txt", tabela_simbolos)
    # Retorna ao final da função para indicar que a execução foi concluída.
    return


# A chamada abaixo inicia o reconhecimento léxico do arquivo de entrada.
# O programa lê entradaCOMP.txt e gera a FITA e a Tabela de Símbolos automaticamente.
reconhecer_arquivo('entradaCOMP.txt')