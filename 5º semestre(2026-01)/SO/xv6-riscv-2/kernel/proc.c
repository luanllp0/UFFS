...
found:
  ...
  p->tickets = 100;                 // Define um valor padrão de bilhetes
  p->stride = 10000 / p->tickets;   // Calcula o passo (Constante / bilhetes) 
  p->pass_value = 0;                // Cada processo inicia com passada zero
...
kfork_tickets(int tickets)
{
  ...
  np->tickets = tickets;               // Define a quantidade de bilhetes solicitada
  np->stride = 10000 / np->tickets;    // Recalcula o passo do filho
  // np->pass_value começa com 0
...
scheduler(void)
{
  ...
  for(;;){ // loop infinito
    ...
    if(selected_p != 0) { // se o processo selecionado é valido
      ...
      // verifica se ainda está pronto para rodar
      if(selected_p->state == RUNNABLE) { 
        
        // incremento da passada do processo com o valor do seu próprio passo
        selected_p->pass_value += selected_p->stride; 
        
        selected_p->state = RUNNING; // coloca no estado executando
        c->proc = selected_p; // coloca o processo na cpu
        swtch(&c->context, &selected_p->context);  // troca de contexto

        // o processo terminou seu tempo na cpu
        c->proc = 0; // cpu não está executando nenhum processo
      }
      release(&selected_p->lock); // solta o lock
    } else {
      // Se não encontrou ninguém para rodar, o processador dorme até a próxima interrupção
      asm volatile("wfi");
    }
  }
}
